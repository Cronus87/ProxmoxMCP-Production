...
setup_claude_user() {
    ...
    # Deploy practical admin sudoers configuration
    log_info "Deploying practical admin configuration..."
    ...
    # Create practical admin sudoers configuration inline
...
# Generated by install.sh - Practical Admin Model
...
# PVE user management for root@pam
...
# Network bridge destruction (vmbr0 is critical)
claude-user ALL=(ALL:ALL) !/usr/sbin/ip link delete vmbr0, !/usr/bin/ip link delete vmbr0

# PVE configuration destruction
claude-user ALL=(ALL:ALL) !/usr/bin/rm -rf /etc/pve, !/bin/rm -rf /etc/pve

# Core Proxmox services
claude-user ALL=(ALL:ALL) !/usr/bin/systemctl stop pveproxy, !/bin/systemctl stop pveproxy
claude-user ALL=(ALL:ALL) !/usr/bin/systemctl stop pvedaemon, !/bin/systemctl stop pvedaemon  
claude-user ALL=(ALL:ALL) !/usr/bin/systemctl stop pvestatd, !/bin/systemctl stop pvestatd

# End of claude-user practical admin configuration
EOF
    ...
    if visudo -c -f "/etc/sudoers.d/claude-user"; then
        log_success "Practical admin configuration deployed"
    else
    ...
    # Validate that broad admin permissions are granted
    if grep -q "/usr/bin/\*" "/etc/sudoers.d/claude-user"; then
        log_success "Practical admin permissions verified"
    else
        log_error "Practical admin permissions missing from sudoers"
        exit 1
    fi
}
...
configure_security() {
    ...
    # Test practical admin permissions specifically
    if sudo -u claude-user sudo -l | grep -q "/usr/bin/\*"; then
        log_success "Practical admin permissions verified for claude-user"
    else
        log_error "Practical admin permissions missing for claude-user"
        exit 1
    fi
